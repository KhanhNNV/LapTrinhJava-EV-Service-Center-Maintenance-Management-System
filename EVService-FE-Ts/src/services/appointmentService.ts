...
export function useUpdateVehicle() {
  ...
  return useMutation({
    ...
    onError: (err: any) =>
      toast.error(err?.response?.data?.message || "Lỗi cập nhật xe"),
  });
}
...
export function useCancelAppointment() {
  ...
  return useMutation({
    ...
    onError: (error: any) => {
      const errorMessage =
        error?.response?.data?.message || "Không thể hủy lịch.";
      toast.error(errorMessage);
    },
  });
}

//--thêm các gói dịch vụ đã đăng ký của khách hàng
export function useCustomerContracts() {
  return useQuery({
    queryKey: ["customer-contracts"],
    queryFn: async () => {
      const res = await api.get("/api/contracts"); // BE đã làm đúng route
      return res.data;
    },
  });
}
