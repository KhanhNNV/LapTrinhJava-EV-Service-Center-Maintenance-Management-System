    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-3xl font-bold tracking-tight">Thông báo Quản trị</h2>
                    <p className="text-muted-foreground">Cập nhật hệ thống và cảnh báo tồn kho</p>
                </div>
            </div>
            ...
            <div className="grid gap-4">
                {notifications?.length > 0 ? (
                    notifications.map((noti: any) => (
                        <Card
                            ...
                            className={`shadow-sm transition-all hover:shadow-md ${
                                !noti.read ? "border-l-4 border-l-red-500 bg-red-50/50" : "opacity-80"
                            }`}
                        >
                            <CardContent className="p-4 flex items-start gap-4">
                                <div className={`p-2 rounded-full ${!noti.read ? "bg-red-100 text-red-600" : "bg-muted text-muted-foreground"}`}>
                                    <Bell className="w-5 h-5" />
                                </div>
                                <div className="flex-1 space-y-1">
                                    <div className="flex justify-between items-start">
                                        <h4 className={`text-base ${!noti.read ? "font-bold text-red-900" : "font-medium text-muted-foreground"}`}>
                                            {noti.title}
                                        </h4>
                    ...
                    <div className="text-center py-12 text-muted-foreground">
                        <p>Hệ thống chưa có thông báo nào</p>
                    </div>
                )}
            </div>
        </div>
