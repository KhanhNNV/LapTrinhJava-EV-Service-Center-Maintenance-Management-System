...
// Định nghĩa Interface khớp với DTO Backend
interface VehicleDto {
    vehicleId: number;
    model: string;
    brand: string;
    licensePlate: string;
    vehicleType: 'ELECTRIC_CAR' | 'ELECTRIC_MOTORBIKE'; // Giả định Enum
    centerId: number;
...
export default function Vehicles() {
    ...
    const addVehicleMutation = useMutation({
        mutationFn: async (data: any) => {
            // Convert centerId to number for backend
        ...
        onError: (error: any) => {
            const msg = error?.response?.data?.message || 'Không thể thêm xe. Vui lòng thử lại.';
            toast.error(msg);
        },
    });
    ...
    const updateVehicleMutation = useMutation({
        mutationFn: async (data: any) => {
            ...
            const response = await api.put(`/api/vehicles/${editingVehicleId}`, payload);
            return response.data;
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['customer-vehicles'] });
        ...
        onError: (error: any) => {
             const msg = error?.response?.data?.message || 'Không thể cập nhật xe.';
            toast.error(msg);
        },
    });
    ...
    const deleteVehicleMutation = useMutation({
        ...
        onError: () => {
            toast.error('Không thể xóa xe. Có thể xe đang có lịch hẹn.');
        },
    });
    ...
    const isSubmitting = addVehicleMutation.isPending || updateVehicleMutation.isPending;

    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-3xl font-bold tracking-tight text-gray-900">Xe của tôi</h2>
                ...
                <Dialog open={isDialogOpen} onOpenChange={(open) => {
                    ...
                    <DialogContent>
                        <DialogHeader>
                            <DialogTitle>{editingVehicleId ? 'Cập nhật thông tin xe' : 'Thêm xe mới'}</DialogTitle>
                            <DialogDescription>
                                {editingVehicleId ? 'Chỉnh sửa thông tin xe điện của bạn' : 'Nhập thông tin xe điện để quản lý bảo dưỡng'}
                            </DialogDescription>
                        </DialogHeader>
                                ...
                                <div className="space-y-2">
                                    ...
                                    >
                                        ...
                                        <SelectContent>
                                            ...
                                            <SelectItem value="ELECTRIC_MOTORBIKE">Xe máy điện</SelectItem>
                                        </SelectContent>
                                    </Select>
                                </div>
                            </div>
            ...
            {isLoading ? (
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    {vehicles.map((vehicle: VehicleDto) => (
                        <Card key={vehicle.vehicleId} className="shadow-sm border-gray-100 hover:shadow-md transition-all hover:-translate-y-1">
                            <CardHeader>
                                <CardTitle className="flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                                            vehicle.vehicleType === 'ELECTRIC_CAR' ? 'bg-blue-100 text-blue-600' : 'bg-green-100 text-green-600'
                                        }`}>
                                            {vehicle.vehicleType === 'ELECTRIC_CAR' ? <Car className="w-5 h-5" /> : <Bike className="w-5 h-5" />}
                                        </div>
                                        <div>
                                            <span className="text-lg block">{vehicle.model}</span>
                                            <span className="text-xs font-normal text-muted-foreground">{vehicle.brand}</span>
                                        </div>
                                    </div>
                                </CardTitle>
                            </CardHeader>
                            <CardContent className="space-y-4">
                                <div className="space-y-2 bg-gray-50 p-3 rounded-md">
                                    ...
                                    <div className="flex justify-between text-sm">
                                        ...
                                        <span className="font-medium">
                                            {vehicle.vehicleType === 'ELECTRIC_CAR' ? 'Ô tô điện' : 'Xe máy điện'}
                                        </span>
                                    </div>
                                    {vehicle.recentMaintenanceDate && (
                                        <div className="flex justify-between text-sm">
                                            <span className="text-muted-foreground">Bảo dưỡng gần nhất:</span>
                                            <span className="font-medium text-blue-600">{vehicle.recentMaintenanceDate}</span>
                                        </div>
                                    )}
                                </div>
                                ...
                                <div className="flex gap-2 pt-2">
                                    ...
                                    <Button
                                        ...
                                        onClick={() => {
                                            if (confirm(`Bạn có chắc muốn xóa xe ${vehicle.model} (${vehicle.licensePlate})?`)) {
                ...
                <Card className="shadow-sm border-dashed border-2 border-gray-200 bg-gray-50/50">
                    <CardContent className="flex flex-col items-center justify-center py-12">
                        ...
                        <h3 className="text-lg font-semibold mb-2 text-gray-900">Chưa có xe nào</h3>
                        <p className="text-muted-foreground mb-4 text-center max-w-sm">
                            Thêm phương tiện của bạn để dễ dàng đặt lịch bảo dưỡng và theo dõi lịch sử dịch vụ
                        </p>
                        <Button className="bg-[#007AFF] hover:bg-[#0066CC]" onClick={() => setIsDialogOpen(true)}>
                            <Plus className="w-4 h-4 mr-2" />
                            Thêm xe ngay
                        </Button>
                    </CardContent>
