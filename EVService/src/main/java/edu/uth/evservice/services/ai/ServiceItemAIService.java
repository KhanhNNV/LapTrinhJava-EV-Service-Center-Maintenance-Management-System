        ...
        try {
            ...
            String aiResponse = chatClient.prompt()
                    .system("""
                        ...
                        YÊU CẦU VỀ ĐỊNH DẠNG OUTPUT (BẮT BUỘC):
                        - KHÔNG được vẽ bảng (No tables).
                        - KHÔNG dùng định dạng in đậm (**) hoặc in nghiêng (*).
                        - KHÔNG dùng tiêu đề Markdown (###).
                        - Chỉ sử dụng văn bản thường và xuống dòng để giải thích lý do.
                        - Viết thành các đoạn văn ngắn hoặc gạch đầu dòng (-) đơn giản.
    ...
    private AIServiceItemSuggestion generateGenericAISuggestions(ServiceItem serviceItem) {
        try {
            String prompt = """
                ...
                Hãy đề xuất 3-5 phụ tùng phổ biến nhất cho dịch vụ này.
                
                YÊU CẦU VỀ ĐỊNH DẠNG CÂU TRẢ LỜI (QUAN TRỌNG):
                1. Trả lời bằng văn bản thuần túy (plain text).
                2. KHÔNG sử dụng bảng (Markdown tables).
                3. KHÔNG sử dụng các ký tự định dạng phức tạp như tiêu đề lớn (###).
                4. Chỉ sử dụng gạch đầu dòng (-) để liệt kê các ý.
                5. Giữ câu trả lời ngắn gọn, tập trung vào lý do đề xuất.
